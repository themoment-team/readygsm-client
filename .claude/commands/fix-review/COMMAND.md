...
### 2단계 — 코드 수정 (코멘트 하나씩 처리)

**코멘트를 한 번에 모두 수정하지 마세요.** 아래 순서대로 코멘트를 하나씩 처리합니다:

1. 첫 번째 코멘트 분석 → 코드 수정 → 3단계(커밋) 및 4단계(답글) 절차 수행
2. 다음 코멘트로 이동 → 반복
...
- 수정 완료: `"<커밋해시> 수정하였습니다."` 형태로 작성
  - 커밋 해시는 `git rev-parse --short HEAD`로 가져온 값 사용
  - 예시: `"83b1ecd 수정하였습니다."`
- 수정 없이 동의: `"저도 그렇게 생각합니다, 반영했습니다."`
- 반려/논의: 이유를 명확히 설명
- 답글은 **한국어**로 작성
